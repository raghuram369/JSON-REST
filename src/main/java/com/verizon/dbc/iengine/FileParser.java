...
public class FileParser {
	...
	private final static Pattern LINE_PATTERN = Pattern.compile("([a-zA-Z]{3}\\s+\\d?\\d\\s+\\d\\d:\\d\\d:\\d\\d)\\s+([^\\.]+?)\\..+?([a-zA-Z]{3}\\s+\\d?\\d\\s+\\d\\d:\\d\\d:\\d\\d):\\s+%(.+?):(\\s+.+?:\\s+)");
	private final static Pattern KEY_PATTERN = Pattern.compile("((.+?)=(.+?):\\s+)|((.+?)=(.+?))$");
	...
	public void parseFile(File file) throws SQLException{
		...
		//Connection conn = ConnectionFactory.getInstance(connProperty).getConnection(connProperty);		
		...
		/*try{
			batchSize = Integer.parseInt(commonProperty.getCommitBatchSize());
		...
		try{
			...
			while (input.hasNextLine()) {
				...
				if (lineMatcher.lookingAt()){
					...
					if(!eventType.equalsIgnoreCase("all") && !eventTypeList.contains(lineMatcher.group(4))){						
						continue;
					}
					
					//eventID = eventDao.insertEvent(conn, fileID, lineMatcher.group(4), lineMatcher.group(2));
										
					keyValue = new HashMap<String, String>();
					end = lineMatcher.end();
					keyMatcher = KEY_PATTERN.matcher(inputLine);
					while(keyMatcher.find(end)){
						if(keyMatcher.group(2) != null && keyMatcher.group(3)!=null){
							keyValue.put(keyMatcher.group(2), keyMatcher.group(3));							
						}else{
							keyValue.put(keyMatcher.group(5), keyMatcher.group(6));
						}
					...
					for(String newExit : keyValue.get("New Exit").split(",")){
			...
			System.out.println("Error in parsing CLI File:" + e.getMessage());
			//eventDao.updateFileStatus(conn, "FAILED", e.getMessage(), fileID);
		} finally {
			if (input != null)
				input.close();
			/*if(conn != null)
